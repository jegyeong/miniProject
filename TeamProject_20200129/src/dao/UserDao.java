...
//	public String ;
	...
	public String getUserName(String userId, String userPass){
		
		
		boolean aabb = checkLoginuser(userId,userPass);
		
		if(aabb){
		for(int i = 0; i < db.userList.size(); i++){
			UserVo a = db.userList.get(i);
			
			if(a.getUserId().equals(userId) && a.getUserPass().equals(userPass))
				return db.userList.get(i).getUserName();
			}
		}
		return null;
